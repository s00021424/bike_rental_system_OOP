...
public class MountainBikeFactory extends BikeFactory {
    private static final Logger logger = Logger.getLogger(MountainBikeFactory.class.getName());
    ...
    @Logged
    @Override
    public MountainBike createBike(BikeBuilder bikeBuilder) {
        try {
            validateBuilder(bikeBuilder);
            return new MountainBike(bikeBuilder);
        } catch (RuntimeException e) {
            logger.severe("Failed to create mountain bike: " + e.getMessage());
            throw new RentalException("Unable to create mountain bike");
        }
    }
}